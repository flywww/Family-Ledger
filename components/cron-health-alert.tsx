...
const severityLabels: Record<NonNullable<CronHealthState["severity"]>, string> = {
  info: "Info",
  warning: "Warning",
  critical: "Critical",
};
...
export default function CronHealthAlert({
  ...
  return (
    <div
      className={`rounded-xl border p-4 ${toneClasses[severity]}`}
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center justify-between gap-3">
          ...
          <p className="text-xs uppercase tracking-wide">
            {severityLabels[severity]} · {health.lastScheduledStatus ?? "no scheduled logs"}
          </p>
        </div>
