...
- Taiwan search requirements map to focused Vitest coverage for listed-stock search helpers or server action parsing, including no paid/web/scraping fallback for unavailable English matches.
- Taiwan source metadata and price-fetch requirements map to focused Vitest coverage for `lib/pricing.ts`.
- Taiwan monthly refresh requirements map to database-backed `tests/monthly-refresh.test.ts` coverage through `npm run test`.
- Existing workflow preservation maps to focused regression tests and `npm run typecheck`, `npm run lint`, and `npm run build`.
- Small UI workflow preservation maps to manual visual review against `docs/design-system.md`; no new reusable component or prototype is required.
